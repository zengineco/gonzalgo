...
/-
  MODIFIED FILE. This is Lean 4's `src/Lean/Elab/Tactic/Omega/Frontend.lean`
  (Lean 4.32.1) copied into an ordinary Lake module and changed, as permitted by
  the Apache 2.0 licence above and noted here in accordance with section 4(b).

  Changes, and nothing else:

    * the eight literal `Classical.propDecidable` arguments are replaced by
      `decInst`, which attempts instance synthesis and falls back to the
      classical instance only when synthesis fails;
    * module-system markers (`module`, `prelude`, `public`) are dropped, since
      this builds as a user module rather than as part of core;
    * the namespace is `OmegaFix`, so the original declarations are untouched;
    * `@[builtin_tactic]` registration is replaced by an ordinary `syntax` /
      `@[tactic]` pair exposing the result as `omega_fix`;
    * a verification section is appended.

  Upstream is unmodified and unaffected. This file exists to demonstrate that
  the proposed fix compiles and produces choice-free proofs; it is not a
  replacement for `omega` and should not be used as one.
-/
...
partial def asLinearCombo (e : Expr) : OmegaM (LinearCombo × OmegaM Expr × List Expr) := do
  ...
  | none =>
    ...
    modifyThe Lean.Elab.Tactic.Omega.Cache fun cache => (cache.insert e (lc, proof.run' cache))
    pure (lc, proof, r)
...
/-- THE PATCH. omega hardcodes `Classical.propDecidable` for the `Decidable`
arguments of its propositional helper lemmas, so goals over `Nat` and `Int` —
where `Nat.decLe`, `Nat.decLt` and `instDecidableEqNat` exist and are axiom-free
— acquire a dependence on `Classical.choice` they never needed.

Ask instance synthesis first; fall back only when it genuinely fails, so
undecidable propositions behave exactly as before. -/
...
syntax (name := omegaFixStx) "omega_fix" : tactic

@[tactic omegaFixStx]
def evalOmegaFix : Tactic
  | `(tactic| omega_fix) => omegaTactic {}
  | _ => throwUnsupportedSyntax


/-! # Verification

Run this file with `lake env lean` against a stock Lean 4.32.1 toolchain. It
builds the patched frontend and then checks it, so the claim and its evidence
travel together.

Nothing here rebuilds Lean core, which needs a C++ toolchain. The frontend is
copied, patched and compiled as an ordinary module, and `omega_fix` is the
patched tactic. Every statement and proof below is verbatim from core, with
`omega` replaced by `omega_fix` and nothing else changed.
-/

section Verify

/-! ## The goal shapes that make stock `omega` classical. -/

theorem p_iff2 (a b : Nat)   : a - b = 0 ↔ a ≤ b           := by omega_fix
theorem p_iff  (m l n : Nat) : min m l ≤ n ↔ m ≤ n ∨ l ≤ n := by omega_fix
theorem p_and  (a b : Nat)   : a ≤ b ∧ b ≤ a ↔ a = b       := by omega_fix
theorem p_imp  (a b : Nat)   : a ≤ b → a ≤ b + 1           := by omega_fix
theorem p_lin  (a b : Nat)   : a - b + b ≥ a               := by omega_fix
theorem p_int  (a b : Int)   : a - b = 0 ↔ a = b           := by omega_fix

-- all six: [propext, Quot.sound]
#print axioms p_iff2
#print axioms p_iff
#print axioms p_and
#print axioms p_imp
...
/-! ## A real core theorem from the cleanable set.
    `Init/Data/Int/DivMod/Lemmas.lean:30`, whose shipped proof is `by omega`. -/

theorem my_natCast_eq_zero {n : Nat} : (n : Int) = 0 ↔ n = 0 := by omega_fix

#print axioms Int.natCast_eq_zero   -- shipped: [propext, Classical.choice, Quot.sound]
#print axioms my_natCast_eq_zero    -- patched: [propext, Quot.sound]

/-! ## Cascade.

A theorem whose proof cites another affected lemma does NOT clean up on its own,
because the lemma it cites still comes from the unpatched toolchain. Rebuilding
the cited lemma too — which is what an actual core rebuild does — propagates the
cleanup. Both statements and proofs are verbatim from
`Init/Data/Int/DivMod/Lemmas.lean:1136` and `:1140`. -/

theorem my_ediv_le_iff_le_mul {k x y : Int} (h : 0 < k) : x / k ≤ y ↔ x < y * k + k := by
  rw [Int.le_iff_lt_add_one, Int.ediv_lt_iff_lt_mul h, Int.add_mul]
  omega_fix

theorem my_le_mul_iff_le_left {x y z : Int} (hz : 0 < z) :
    x ≤ y * z ↔ (x + z - 1) / z ≤ y := by
  rw [my_ediv_le_iff_le_mul hz]     -- the rebuilt lemma, not the shipped one
  omega_fix

#print axioms Int.ediv_le_iff_le_mul   -- shipped: Classical.choice
#print axioms Int.le_mul_iff_le_left   -- shipped: Classical.choice
#print axioms my_ediv_le_iff_le_mul    -- rebuilt: [propext, Quot.sound]
#print axioms my_le_mul_iff_le_left    -- rebuilt: [propext, Quot.sound]
